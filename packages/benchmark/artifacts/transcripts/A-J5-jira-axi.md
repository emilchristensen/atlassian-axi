<!-- provenance: measured accuracy run (plan-only); executor final message verbatim (then redacted per src/redact.ts); task A-J5, surface jira-axi, executor model claude-fable-5, 2026-08-05 -->
# Transcript
## $ node <repo>/packages/jira-axi/dist/bin/jira-axi.js workitem comment --help
```
...
```
# PLANNED COMMAND
node <repo>/packages/jira-axi/dist/bin/jira-axi.js workitem comment TEAM-40 --body "benchmark probe"
# FINAL ANSWER
The command will add a comment with the text "benchmark probe" to Jira work item TEAM-40 (the markdown body is stored as ADF).
