...
**Jira half facts (Phase 2).** acli v1.3.22 has NO `workitem list` subcommand - our `list` builds JQL and calls `workitem search`; comments live under the `workitem comment <create|list|...>` group. acli's `--json` payload shape is undocumented: fixtures in `test/fixtures/acli.ts` are hand-verified against the Jira Cloud REST shape (provenance in the file header - update them together with the tolerant accessors in `src/commands/jira/shared.ts` if a live capture ever disagrees). Mutations are `--yes`-gated non-interactive, re-fetch via `workitem view` after mutating, and never parse mutation output beyond a tolerant created-key probe.
